...
- `carrier-board.kicad_pro` / `carrier-board.kicad_sch` — top sheet, just
  ...
  - `clocking.kicad_sch` — reference clock synthesizer(s), one
    independent output per quad used (3 inputs + 1 output, no sharing —
    see carrier-board-spec.md for why cross-quad sharing was ruled out)
...
The GTH cross-quad clocking question went through two rounds: an earlier
pass concluded 4-in+1-out fit cleanly by having HDMI OUT borrow lanes
from 3 quads; a closer look at the actual CPLL math (UG576 Equation
2-1/2-2) found that only works if OUT's rate happens to be one of ~29
discrete ratios away from whatever the shared input's live reference
currently is — a real conflict with "OUT must also support arbitrary
custom rates." Corrected conclusion in carrier-board-spec.md: **3 fully
independent native inputs + 1 fully independent native output** fit
cleanly (one quad each, no sharing); **the 4th input needs a decision**
(defer to a second board, accept a compromise on one port, or a
different device) that hasn't been made yet. Real schematic capture
(GTH pin assignments) shouldn't start until that decision is made.

## Next steps

1. **Decide how to handle the 4th input** — the main open decision, see
...
3. Decide the reference clock synthesizer part for `clocking.kicad_sch`
   (4 independent outputs needed, one per quad).
4. Populate `hdmi_in*.kicad_sch` / `hdmi_out.kicad_sch` with actual
   connector + support circuitry once 1-3 are settled.
5. Confirm the GTH allocation at the Vivado pin-planner level once pins
